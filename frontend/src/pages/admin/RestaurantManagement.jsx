...
const RestaurantManagement = () => {
  ...
  const fetchMerchants = async () => {
    try {
      ...
      const { data } = await axios.get(((import.meta.env.VITE_BACKEND_URL || (import.meta.env.PROD ? '' : 'http://localhost:5000')) + '/api/admin/users?role=Merchant'), config);
      setMerchants(data);
    } catch (error) {
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-end mb-8">
        <div>
          ...
          <p className="text-slate-500 mt-2">Approve registrations, verify partners, and manage active restaurants.</p>
        </div>
      </div>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100">
                {merchants.map((merchant) => (
                  <tr key={merchant._id} className="hover:bg-slate-50 transition-colors">
                    <td className="p-4 pl-6">
                      <div className="flex items-center">
                        <div className="w-10 h-10 rounded-xl bg-orange-100 flex items-center justify-center text-orange-600 font-black mr-4 uppercase">
                          <Store className="w-5 h-5" />
                        </div>
                        <div>
                          <p className="font-bold text-slate-900">{merchant.businessName || merchant.name}</p>
                    ...
                    <td className="p-4 pr-6 text-right space-x-2">
                      <button 
                        onClick={() => toggleVerification(merchant._id, merchant.isVerified)}
                      ...
                      >
                        {merchant.isVerified ? 'Revoke' : 'Verify'}
                      </button>
                      <button 
                        onClick={() => toggleBlockStatus(merchant._id, merchant.isBlocked)}
                      ...
                      >
                        {merchant.isBlocked ? 'Unblock' : 'Suspend'}
                      </button>
                    </td>
                  </tr>
                ))}
